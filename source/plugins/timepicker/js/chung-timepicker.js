	...
	$.chungTimePicker = function(element, options) {
		...
		function createHoursWrap() {
			var table = $('<table class="table-condensed"></table>'),
				title = $('<thead><tr><th colspan="6" class="title">Выберите час</th></tr></thead>'),
				hours = $('<tbody></tbody>'),
		...
		function createMinutesWrap() {
			...
			switch(obj.options.viewType) {
				case 0:
					title.find('tr').append('<th class="prev js-back-hours"><i class="icon-arrow-left"></i></th><th colspan="4" class="title">Выберите минуту</th>');
					break;
				case 1:
					title.find('tr').append('<th colspan="6" class="title">Выберите минуту</th>');
					break;
				default:
					console.log(obj.errorPrefix + 'viewType Error');
					break;
		...
		function createBottomOperate() {
			switch(obj.options.viewType) {
				case 0:
					obj.timepicker_bottom.empty().append('<span class="bottom-btn js-clear">Очистить</span>')
						.append('<span class="bottom-btn js-cancel">Отменить</span>');
					break;
				case 1:
					obj.timepicker_bottom.empty().append('<span class="bottom-btn js-clear">Очистить</span>')
						.append('<span class="bottom-btn js-cancel">Отменить</span>')
						.append('<span class="bottom-btn js-confirm">ОК</span>');
